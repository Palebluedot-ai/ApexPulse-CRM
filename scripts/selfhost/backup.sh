...
#
# 可用环境变量覆盖默认值：
#   PG_CONTAINER     docker postgres 容器名（默认 hashkey-otc-crm-v1-postgres）
#   PG_USER          数据库用户（默认 postgres）
#   PG_DB            数据库名（默认 hashkey_otc_crm_v1）
...
PG_CONTAINER="${PG_CONTAINER:-hashkey-otc-crm-v1-postgres}"
PG_USER="${PG_USER:-postgres}"
PG_DB="${PG_DB:-hashkey_otc_crm_v1}"
